...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form action="" method="post">
            ...
            <div class="form-group">
                ...
                <?php new \app\widgets\menu\Menu([
                    ...
                    'tpl' => APP . '/widgets/menu/admin_select_tpl.php', // подключаем данный шаблон
                ]) ?>
            </div>
            ...
            <div class="card card-info card-outline card-tabs">
                <div class="card-header p-0 pt-1 border-bottom-0">
                    <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                        <?php foreach (\wfm\App::$app->getProperty('languages') as $k => $lang): ?> <!-- тут мы выводим языки на которых нужно добавить категории; $k это ид языка -->
                            <li class="nav-item">
                ...
                <?= debug(\wfm\App::$app->getProperty('languages')) ?>
                <!-- get_field_array_value()-нам нужна, для того чтобы выводить данные которые были введены и не потерять их если возникла ошибка валидации -->
                    ...
                    <div class="tab-content">
                        <?php foreach (\wfm\App::$app->getProperty('languages') as $k => $lang): ?>
                            <div class="tab-pane fade <?php if ($lang['base']) echo 'active show' ?>" id="<?= $k ?>">
                                ...
                                <!-- get_field_array_value() с помощью данного метода мы берем значение index которое пришло к нам методом POST -->
                                <div class="form-group">
                                    ...
                                    <input type="text" name="category_description[<?= $lang['id'] ?>][title]"
                                           ...
                                           required2> <!-- НУЖНО ПОСТАВИТЬ ТУТ required -->
                                </div>
        ...
        <?php // если существуют в сессии данные который ввел пользователь, то удаляем их
...
<script> // интеграция ckeditor
    ...
    window.editors = {};
    document.querySelectorAll('.editor').forEach((node, index) => { // ищем все что у нас есть в классе editor
        ClassicEditor
            .create(node, { // создаем визуальный редактор
                ckfinder: {
