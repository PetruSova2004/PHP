...
class Download extends AppModel
{
    ...
    public function download_validate(): bool
    {
        ...
        if (empty($_FILES) || $_FILES['file']['error']) {
            ...
            $ext = end($parts); // получим то что написано после '.'; Пример: image.jpg
    ...
    public function upload_file(): array|false // загружаем файл в указанную папку
    {
        // uniqid — Сгенерировать уникальный ID
        // move_uploaded_file — Перемещает загруженный файл в новое место
        $file_name = $_FILES['file']['name'] . uniqid(); // к имени файла добавляем рандомный ид
        $path = WWW . '/downloads/' . $file_name;
        if (move_uploaded_file($_FILES['file']['tmp_name'], $path)) { // перемещаем файл
            return [
