...
class Product extends AppModel
{
    ...
    public function get_products($lang, $start, $perpage): array // получаем все продукты
    {
    ...
    public function product_validate(): bool
    {
        ...
        foreach ($_POST['product_description'] as $lang_id => $item) {
            // exerpt это то что попало в Краткое описание
    ...
    {
        $lang = App::$app->getProperty('language')['id']; // ид активного языка
        R::begin();
        try {
            // Выгружаем всё в product
            $product = R::dispense('product');
            $product->category_id = post('parent_id', 'i');
            $product->price = post('price', 'f');
            $product->old_price = post('old_price', 'f');
            $product->status = post('status') ? 1 : 0; // если у нас есть status тогда запишем 1, в противном случае тут будет 0
            $product->hit = post('hit') ? 1 : 0;
            $product->img = post('img') ?: NO_IMAGE; // в POST('img') попадет маршрут на изображение
            $product->is_download = post('is_download') ? 1 : 0;
            $product_id = R::store($product); // метод store вернёт ид добавленной записи

            $product->slug = AppModel::create_slug('product', 'slug', $_POST['product_description'][$lang]['title'], $product_id);
            R::store($product);

            // Выгружаем всё в product_description
            foreach ($_POST['product_description'] as $lang_id => $item) {
            ...
            // Выгружаем всё в product_gallery if exists
            if (isset($_POST['gallery']) && is_array($_POST['gallery'])) {
                // если мы прилепляли дополнительно изображения, то они попадут в $_POST['gallery']
                $sql = "INSERT INTO product_gallery (product_id, img) VALUES ";
                foreach ($_POST['gallery'] as $item) { // $item это картиночки
                    $sql .= "({$product_id}, ?),";
                }
                $sql = rtrim($sql, ',');
            ...
            // Выгружаем всё в product_download if is_download
    ...
    public function update_product($id): bool
    {
        ...
        try {
            // product
            $product = R::load('product', $id); // получаем объект продукта по $id
            if (!$product) {
                return false;
            }
            $product->category_id = post('parent_id', 'i');
            ...
            if (!isset($_POST['gallery'])) { // если пользователь нам ничего не прислал в качестве галереи картинок при редактировании
                R::exec("DELETE FROM product_gallery WHERE product_id = ?", [$id]); // пытаемся их удалить, не важно если были они там или нет
            }

            if (isset($_POST['gallery']) && is_array($_POST['gallery'])) { // если что-то к нам пришло в качестве картинок
                $gallery = self::get_gallery($id); // получаем все картинки галереи

                // array_diff — Проверяет если есть разница между массивами
            ...
            R::exec("DELETE FROM product_download WHERE product_id = ?", [$id]); // удаляем всё что касается цифрового товара
            if ($product->is_download) { // если прикреплён товар
                $download_id = post('is_download', 'i');
                R::exec("INSERT INTO product_download (product_id, download_id) VALUES (?,?)", [$product_id, $download_id]);
            }
    ...
    public function get_product($id): array|false
    {
        ...
        $key = key($product); // берем ключ первого массива $product
        if ($product[$key]['is_download']) { // если товар цифровой
            $download_info = self::get_product_download($id);
//            debug($download_info, 1);
    ...
    public function get_product_download($product_id): array // получаем инфо о прикрепленном файле
    {
        $lang_id = App::$app->getProperty('language')['id']; // ид активного языка
